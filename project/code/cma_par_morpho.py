...
from morpho import mutate_morphology
...
if __name__ == "__main__":
    ...
    robot_tested = {}
    for i in range(5):
        new_gen = [mutate_morphology(best) for _ in range(10)] + [best]
        
        for robot in new_gen:
            robot_key = tuple(map(tuple, robot))

            if robot_key not in robot_tested:
                best_trained, fitness, cfg = run_cma_par(robot, gen_counter=5, max_steps=300, popsize=5, sigma0=1)
                robot_tested[robot_key] = (best_trained, fitness, cfg)
            
        sorted_robots = sorted(robot_tested.items(), key=lambda x: x[1][1], reverse=True)
